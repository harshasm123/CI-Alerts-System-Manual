...
EMA_SOURCES = [
    {
        'name': 'EMA Medicine Approvals',
        'url': 'https://www.ema.europa.eu/en/rss.xml',
        'type': 'rss'
    },
    {
        'name': 'EMA Safety Updates',
        'url': 'https://www.ema.europa.eu/en/rss/safety-updates.xml',
        'type': 'rss'
    },
    {
        'name': 'EMA News',
...
def handler(event, context):
    """
    Fetch recent EMA announcements and filter for pharmaceutical molecules
    """
    try:
        results = []
        ...
        for source in EMA_SOURCES:
            try:
                        ...
                        if relevant_molecules:
                            for molecule in relevant_molecules:
                                ...
                                sqs.send_message(
                                    ...
                                    MessageAttributes={
                                        'source': {'StringValue': 'EMA', 'DataType': 'String'},
                ...
                print(f"Error processing EMA source {source['name']}: {str(e)}")
                continue
        
        # Store raw data in S3
        s3_key = f"raw/ema/{datetime.now().strftime('%Y/%m/%d')}/{datetime.now().strftime('%H%M%S')}.json"
        s3.put_object(
            Bucket=DATA_BUCKET,
            Key=s3_key,
        ...
        return {
            'statusCode': 200,
            'body': json.dumps({
                'message': f'Successfully processed {len(results)} EMA items',
        ...
        print(f"Error in EMA ingestion: {str(e)}")
        return {
            'statusCode': 500,
